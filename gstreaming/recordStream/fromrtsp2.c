...
int main(int argc, char *argv[]) {
  ...
  char filename[52] = "/home/kim/projectOWL/gstreaming/clips/%s.mkv", asctime(timeinfo); */
  ...
  if (!data.pipeline || !data.rtspsrc || !data.matroskamux || !data.rtph264depay || !data.h264parse || !data.filesink ) {
    ...
    g_printerr ("--All elements was created equally.\n");
  }

  /* Set parameters for some elements */
	g_object_set(G_OBJECT(data.rtspsrc), "location", "rtsp://root:hest1234@192.168.130.200/axis-media/media.amp", "ntp-sync", TRUE, "protocols", 0x00000004, NULL);
	g_object_set(G_OBJECT(data.capsfilter), "caps", gst_caps_from_string("application/x-rtp,media=video"), NULL);
  g_printerr ("---capsfilter.\n");
	g_object_set(G_OBJECT(data.filesink), "location", "/home/kim/projectOWL/gstreaming/clips/11dec_1.mkv", NULL);
  g_printerr ("---filesink.\n");
