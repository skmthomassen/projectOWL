...
int fifo;
char buf[128] = "";
char fifo_name[] = "/data/gstplayer/gst.fifo";
char files[255][255];
...
void player_seek_absolute(int position)
{
...
void player_play(char *filename)
{
	...
	// Create new pipeline
	pipeline = gst_parse_launch("filesrc name=my_filesrc typefind=true ! aiurdemux name=demux demux. ! queue max-size-buffers=300 max-size-time=0 ! vpudec ! mfw_v4lsink demux. ! queue max-size-buffers=0 max-size-time=0 ! mfw_aacdecoder ! alsasink device=hw:2", &error);
	...
	if (dir != NULL) {
		...
		while ((ent = readdir (dir)) != NULL) {
			...
			if(strncmp(ent->d_name, ".", 1024) > 0 && strncmp(ent->d_name, "..", 1024) > 0) {
				sprintf(files[i], "/data/mp4/%s", ent->d_name);
				i++;
			}
		}
		strcpy(files[i+1], "");
...
gboolean timeout(gpointer data)
{
	...
	if(read(fifo, &buf, sizeof(char)*128) > 0)
	{
		...
		if(strcmp(buf, "pause") == 0) {
			...
			if(pipeline->current_state == GST_STATE_PLAYING || pipeline->current_state == GST_STATE_PAUSED) {
				player_stop();
			 }
			...
			sprintf(filename, "/data/mp4/%s.mp4", content_identifier);
